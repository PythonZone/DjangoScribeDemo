...
from cyberhotel.models import Residence,Salle,SalleDeBain,Chambre,Personne,Resident,Locataire,Location,Reduction
...
def htmlTooltip(o):
  ...
  if hasattr(o._meta,"computed_fields"):
    ...
    for opname in o._meta.computed_fields:
      ...
      if isinstance(value,(tuple,list,QuerySet)) and not isinstance(value, basestring):
...
def objectToURL(o):
  ...
  # tooltip = "coucou"
  return format_html("<a href='"+url+"' class='tooltip'>"+str(o)+"<span>"+tooltip+"</span></a>")
...
class ChambreInLine(admin.StackedInline):
  ...
  inline_classes = ('grp-collapse grp-closed',)   # grappelli # only for stackedInline
  fields=['numero','etage','enTravaux','nbLitsSimples','nbLitsDoubles','prix','estNonFumeur']
  extra = 0
...
class ResidenceAdmin(admin.ModelAdmin): #ImportExportModelAdmin):
  ...
  class Meta:
    permissions = (
      ("viewonly_residence", "View only residence (require change_residence")
    )
  ...
  fieldsets = [
      ...
      (None,  #ignored
        {'fields':  (),   # grappelli   
         ...
         'description': 'this is some text with <em>bold</em><br/>toto'}  #ignored
         ),  
      ('Additional information', 
        {'fields': [('nbPlacesMax','tarifMoyen')],
         'classes': ['grp-collapse grp-closed']}), #grappelli
    ]
  radio_fields = {"categorie": admin.HORIZONTAL}  # alternative to dropdown or raw_id for Choice/Dropdown
  
  #-- list view
  actions_on_top = True; actions_on_bottom = False  # no effect with grappelli
  inlines = [SalleInline,SalleDeBainInLine,ChambreInLine]
  list_display =  ['nom','categorie','etageMin','etageMax','nbDeSalles','displayChambres','displaySallesDeBain','displayChambresUtiles']
...
class ResidentAdmin(admin.ModelAdmin):
  #-- list view
  list_display = ['nom','genre','age','estFumeur','conjoint','residence','chambreOccupee']
  list_editable = ['genre','age','estFumeur','conjoint','chambreOccupee']
  list_filter = ['age','genre','estFumeur','chambreOccupee']
  c = ['nom']
  #-- edit view
  fields=[("nom","age","genre"),("chambreOccupee","estFumeur"),'conjoint','tuteurs']
  # inlines = [ EstResponsableDeInline, ]
  
  
  
class SalleAdmin(admin.ModelAdmin):
  def displayResidence(self,instance): return objectToURL(instance.residence)
...
class SalleDeBainAdmin(admin.ModelAdmin):
  #-- list view
  list_display=['__unicode__','residence','numero','etage','enTravaux','estSurLePallier','chambre']
  list_display_links=['__unicode__']
  #-- edit view
...
class ChambreAdmin(admin.ModelAdmin):
  ...
  def displayOccupants(self,instance): return objectsToURL(instance._occupants.all())
  ...
  readonly_fields = ['nbDePlaces','occupants','occupantsList','displayResidence','displayOccupants']
  #-- list view
  list_display=['__unicode__','displayResidence','etage','enTravaux','nbLitsSimples','nbLitsDoubles','prix','estNonFumeur','nbDeOccupants','nbDePlaces','displayOccupants']
  list_display_links=['__unicode__']
  # change_list_template = "admin/change_list_filter_sidebar.html"  # grappelli 
  list_filter = ['residence','residence__categorie',EtageTypeListFilter,'etage','enTravaux','nbLitsSimples','nbLitsDoubles','estNonFumeur']
  search_fields = ['prix','residence__nom']
  ordering=['residence','numero']
  list_per_page=10
  #-- edit view
  fields = ['residence',('numero','etage'),'enTravaux',('nbLitsSimples','nbLitsDoubles'),'estNonFumeur','prix','nbDePlaces','displayOccupants']
